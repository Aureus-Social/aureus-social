...
// ═══════════════════════════════════════════════════════════════════
export function PayrollSimulatorAdvanced({ calcPayroll, LOIS_BELGES, props_tab }) {
  const { t, lang, tText } = useLang();
  ...
  const CP_BAREMES = {
    ...
    '121':{ label:'CP 121 — Nettoyage',                min: BAREMES_CP_MIN['121']?.cl1 || (RMMMG - 40.60) },
    ...
    '302':{ label:'CP 302 — Hôtels & restaurants',    min: BAREMES_CP_MIN['302']?.cl1 || (RMMMG - 40.60) },
    ...
    '330':{ label:'CP 330 — Santé (non-marchand)',     min: BAREMES_CP_MIN['330']?.cl1 || RMMMG },
    ...
    '152':{ label:'CP 152 — Enseignement libre',       min: BAREMES_CP_MIN['152']?.cl1 || (RMMMG - 40.60) },
  };
  ...
  const Controls = () => React.createElement('div', { style: { display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr 1fr', gap: 10, marginBottom: 16 } },
    ...
    React.createElement('div', null,
      React.createElement('div', { style: labelS }, 'Commission Paritaire'),
      React.createElement('select', { value: cp, onChange: e => setCp(e.target.value), style: inputS },
        Object.entries(CP_BAREMES).map(([k, v]) =>
