...
export function GuidePortailMod({s,d}){
  ...
  return <div>
    ...
    {tab==='calendrier'&&<div>
      ...
      {[
        {m:'Chaque mois',t:[{l:'Provisions ONSS',d:'5 du mois suivant',c:'#4ade80'},{l:'Precompte Pro 274',d:'15 du mois suivant',c:'#60a5fa'},{l:tText('Fiches de paie'),d:'Jour du paiement',c:'#c6a34e'}]},
        ...
        {m:tText('Annuel'),t:[{l:'Bilan Social BNB',d:'28 fevrier 2026',c:'#fb923c'},{l:'Belcotax 281',d:'1 mars 2026',c:'#f87171'},{l:'Compte individuel',d:'1 mars 2026',c:'#fb923c'},{l:'Vacances annuelles (attestation)',d:'31 mars 2026',c:'#fb923c'}]},
        {m:'Ponctuel',t:[{l:tText('Dimona IN'),d:'AVANT debut travail',c:'#f87171'},{l:tText('Dimona OUT'),d:'1er jour ouvrable apres fin',c:'#f87171'},{l:'DRS (risques sociaux)',d:'5 jours ouvrables',c:'#fb923c'}]},
    ...
    <PH title="Frais de gestion" sub={`Grille tarifaire — ${totalItems} prestations · ${filledItems} tarifs définis`} actions={<div style={{display:'flex',gap:10}}>
      ...
      <B onClick={()=>d({type:"MODAL",m:{w:900,c:<div>
        ...
        <div style={{display:'flex',gap:10,marginTop:12,justifyContent:'flex-end'}}>
          ...
          <B onClick={()=>{navigator.clipboard?.writeText(exportGrille());alert('Copié !')}}>{tText('Copier')}</B>
        </div>
      </div>}})}>📄 Aperçu grille</B>
    </div>}/>
  ...
  return <div>
    ...
    {tab==='detail'&&<C>
      <Tbl cols={[
        ...
        {k:'st',l:tText('Statut'),r:r=><span style={{fontSize:10,padding:'2px 6px',borderRadius:4,background:r.isO?'rgba(248,113,113,.1)':'rgba(96,165,250,.1)',color:r.isO?'#f87171':'#60a5fa'}}>{r.isO?'Ouvrier':'Employe'}</span>},
        ...
        {gen&&<C style={{marginTop:16}}>
          ...
          <div style={{display:'flex',gap:10,marginTop:12}}>
            ...
            <B v="ghost" onClick={()=>d({type:"MODAL",m:{w:1000,c:<div>
              ...
              <div style={{display:'flex',gap:10,marginTop:12,justifyContent:'flex-end'}}>
                <B v="outline" onClick={()=>d({type:"MODAL",m:null})}>{tText('Fermer')}</B>
                <B onClick={()=>{navigator.clipboard?.writeText(gen);alert('Copié !')}}>{tText('Copier')}</B>
              </div>
            </div>}})}>🔍 Plein écran</B>
          </div>
        </C>}
  ...
  const showCI=(emp)=>{
    ...
    d({type:"MODAL",m:{w:900,c:<div>
      ...
      <div style={{display:'flex',gap:10,marginTop:14,justifyContent:'flex-end'}}>
        <B v="outline" onClick={()=>d({type:"MODAL",m:null})}>{tText('Fermer')}</B>
        <B onClick={()=>{navigator.clipboard?.writeText(doc);alert('Copié !')}}>{tText('Copier')}</B>
      </div>
    </div>}});
  };
...
export function AccountingOutputMod({s,d,supabase,user}){
  ...
  const pcmnLabels={brut:'Rémunérations brutes',onssE:tText('ONSS patronal'),onssW:tText('ONSS travailleur'),pp:tText('Précompte professionnel'),net:'Net à payer',cheqRepas:'Chèques-repas',fraisDeplacement:'Frais de déplacement',provisions:'Provisions sociales',maladieGM:'Assurance groupe/maladie',peculeVacances:'Pécule de vacances',primeFinAnnee:'Prime de fin d\'année',avantagesNature:tText('Avantages en nature')};
  ...
  return <div style={{padding:24}}>
      ...
      <div style={sCard}>
        ...
        <div style={{display:'flex',gap:12,alignItems:'center',flexWrap:'wrap'}}>
          <div style={{display:'flex',gap:4}}>
            {[{id:'month',l:tText('Mensuel')},{id:'quarter',l:'Trimestriel'},{id:'year',l:tText('Annuel')}].map(p=><button key={p.id} onClick={()=>setPeriodType(p.id)} style={sBtn(periodType===p.id)}>{p.l}</button>)}
          </div>
    ...
    {tab==='history'&&<div style={sCard}>
      ...
      {historyLoading?<div style={{padding:20,textAlign:'center',color:'#666'}}>{tText('Chargement...')}</div>:
        ...
        <table style={{width:'100%',borderCollapse:'collapse',fontSize:11}}>
          <thead><tr><th style={sTh}>{tText('Date')}</th><th style={sTh}>{tText('Format')}</th><th style={sTh}>{tText('Période')}</th><th style={sTh}>Client</th><th style={sTh}>Lignes</th><th style={sTh}>{tText('Brut')}</th><th style={sTh}>Net</th></tr></thead>
    ...
    {tab==='preview'&&previewData&&<div style={sCard}>
      ...
      <table style={{width:'100%',borderCollapse:'collapse',fontSize:11}}>
        <thead><tr><th style={sTh}>Client</th><th style={sTh}>Employé</th><th style={sTh}>{tText('Brut')}</th><th style={sTh}>ONSS T.</th><th style={sTh}>Précompte</th><th style={sTh}>Net</th></tr></thead>
...
export function MarketplaceMod({s,d}){
  ...
  const cats=[{v:'all',l:tText('Tous')},{v:'rh',l:'RH'},{v:'finance',l:'Finance'},{v:'temps',l:'Temps'},{v:'formation',l:tText('Formation')},{v:'mobilite',l:'Mobilité'},{v:'documents',l:tText('Documents')},{v:'compliance',l:'Compliance'},{v:'analytics',l:tText('Analytics')},{v:'ia',l:'IA'}];
...
export function AutomationsMod({s,d}){
  ...
  const triggerLabels={date:tText('Mensuel'),date_quarterly:'Trimestriel',date_yearly:tText('Annuel'),event:'Événement',condition:'Condition',daily:'Quotidien'};
...
export function SchedulerMod({s,d}){
  ...
  const tasks=[
    // Mensuels
    {id:'T1',title:'Calcul fiches de paie',deadline:`${y}-${String(m).padStart(2,'0')}-25`,freq:tText('Mensuel'),cat:'Paie',status:m>1?'done':'pending',prio:'haute',auto:true},
    {id:'T2',title:'Provisions ONSS',deadline:`${y}-${String(m).padStart(2,'0')}-05`,freq:tText('Mensuel'),cat:tText('ONSS'),status:'done',prio:'haute',auto:true},
    {id:'T3',title:'Virements salaires (SEPA)',deadline:`${y}-${String(m).padStart(2,'0')}-28`,freq:tText('Mensuel'),cat:'Paie',status:'pending',prio:'haute',auto:false},
    {id:'T4',title:`PP 274 — ${m>1?new Date(y,m-2).toLocaleString('fr',{month:'short'}):new Date(y-1,11).toLocaleString('fr',{month:'short'})}/${m>1?y:y-1}`,deadline:`${y}-${String(m).padStart(2,'0')}-15`,freq:tText('Mensuel'),cat:'Fiscal',status:now.getDate()>15?'done':'pending',prio:'haute',auto:true},
    // Trimestriels
    {id:'T5',title:`DmfA T${q>1?q-1:4}/${q>1?y:y-1}`,deadline:q===1?`${y}-01-31`:q===2?`${y}-04-30`:q===3?`${y}-07-31`:`${y}-10-31`,freq:'Trimestriel',cat:tText('ONSS'),status:'pending',prio:'critique',auto:true},
    {id:'T6',title:`Solde ONSS T${q>1?q-1:4}/${q>1?y:y-1}`,deadline:q===1?`${y}-01-31`:q===2?`${y}-04-30`:q===3?`${y}-07-31`:`${y}-10-31`,freq:'Trimestriel',cat:tText('ONSS'),status:'pending',prio:'critique',auto:false},
    // Annuels
    {id:'T7',title:`Belcotax 281 — ${y-1}`,deadline:`${y}-02-28`,freq:tText('Annuel'),cat:'Fiscal',status:m>2?'done':'pending',prio:'critique',auto:true},
    {id:'T8',title:`Bilan social ${y-1}`,deadline:`${y}-03-31`,freq:tText('Annuel'),cat:'RH',status:'pending',prio:'moyenne',auto:false},
    {id:'T9',title:`Indexation salariale ${y}`,deadline:`${y}-01-15`,freq:tText('Annuel'),cat:'Paie',status:m>1?'done':'pending',prio:'haute',auto:true},
    {id:'T10',title:`Plan de formation ${y}`,deadline:`${y}-03-31`,freq:tText('Annuel'),cat:'RH',status:'pending',prio:'moyenne',auto:false},
    {id:'T11',title:`Fiches 281.10 travailleurs`,deadline:`${y}-02-28`,freq:tText('Annuel'),cat:'Fiscal',status:m>2?'done':'pending',prio:'critique',auto:true},
    {id:'T12',title:`Fiche 281.20 dirigeants`,deadline:`${y}-02-28`,freq:tText('Annuel'),cat:'Fiscal',status:m>2?'done':'pending',prio:'critique',auto:true},
    {id:'T13',title:'Vérification contrats CDD',deadline:`${y}-${String(m).padStart(2,'0')}-01`,freq:tText('Mensuel'),cat:tText('Contrats'),status:'pending',prio:'moyenne',auto:true},
    {id:'T14',title:'Médecine du travail — planification',deadline:`${y}-${String(m).padStart(2,'0')}-15`,freq:tText('Annuel'),cat:'Bien-être',status:'pending',prio:'moyenne',auto:false},
  ];
  ...
  return <div>
    ...
    <div style={{display:'flex',gap:6,marginBottom:14,flexWrap:'wrap'}}>
      {['all','pending','done','Paie',tText('ONSS'),'Fiscal','RH',tText('Contrats')].map(f=><button key={f} onClick={()=>setFilter(f)} style={{padding:'5px 12px',borderRadius:16,border:filter===f?'1px solid rgba(198,163,78,.3)':'1px solid rgba(198,163,78,.06)',background:filter===f?'rgba(198,163,78,.1)':'transparent',color:filter===f?'#c6a34e':'#9e9b93',cursor:'pointer',fontSize:10.5,fontFamily:'inherit'}}>{{all:tText('Tous'),pending:'À faire',done:'Terminées'}[f]||f}</button>)}
    </div>
    <C>
      <table style={{width:'100%',borderCollapse:'collapse',fontSize:11}}>
...
export function TemplatesMod({s,d}){
  ...
  const templates=[
    {id:'tpl_cdi',name:tText('Contrat CDI'),cat:tText('Contrat'),icon:'📄',desc:'Contrat à durée indéterminée conforme à la loi du 3/7/1978',fields:[tText('Nom'),tText('Prénom'),'NISS','Fonction','CP',tText('Salaire brut'),'Date début','Lieu de travail','Horaire']},
    {id:'tpl_cdd',name:tText('Contrat CDD'),cat:tText('Contrat'),icon:'📄',desc:'Contrat à durée déterminée — écrit obligatoire AVANT le début',fields:[tText('Nom'),tText('Prénom'),'NISS','Fonction','CP',tText('Salaire brut'),'Date début','Date fin',tText('Motif')]},
    {id:'tpl_student',name:'Convention étudiant (COE)',cat:tText('Contrat'),icon:'🎓',desc:'Convention d\'occupation étudiant — mentions obligatoires (Titre VII)',fields:[tText('Nom'),tText('Prénom'),'NISS','Fonction','Salaire horaire','Date début','Date fin','Horaire','École']},
    ...
    {id:'tpl_c4',name:tText('Formulaire C4'),cat:'Sortie',icon:'📋',desc:'Certificat de chômage — obligatoire à la fin du contrat',fields:[tText('Nom'),'NISS','Date entrée','Date sortie','Motif fin','Dernier salaire']},
    {id:'tpl_préavis',name:'Lettre de préavis',cat:'Sortie',icon:'✉️',desc:'Notification de préavis par l\'employeur (recommandé)',fields:['Nom employé',tText('Ancienneté'),'Durée préavis','Date début préavis',tText('Motif')]},
    ...
    {id:'tpl_attestation',name:'Attestation d\'emploi',cat:'Attestation',icon:'✅',desc:'Certificat confirmant l\'emploi (banque, bail, etc.)',fields:[tText('Nom'),'Fonction','Date entrée','Type contrat',tText('Salaire brut')]},
    {id:'tpl_telework',name:'Avenant télétravail',cat:tText('Contrat'),icon:'🏠',desc:'Avenant télétravail structurel conforme CCT n°85',fields:['Nom employé','Jours télétravail','Indemnité bureau','Équipement fourni','Date début']},
    {id:'tpl_rt',name:tText('Règlement de travail'),cat:'Légal',icon:'📖',desc:'Document obligatoire dès le 1er travailleur (Loi 8/4/1965)',fields:['Horaires','Modes de paiement','Sanctions',tText('Préavis'),'Premiers secours','CPPT']},
    {id:'tpl_nda',name:tText('Accord de confidentialité'),cat:'Légal',icon:'🔒',desc:'Clause de confidentialité pour données sensibles',fields:['Nom employé','Périmètre',tText('Durée'),'Sanctions',tText('Date')]},
    {id:'tpl_warning',name:'Avertissement écrit',cat:'Disciplinaire',icon:'⚡',desc:'Premier ou second avertissement formel',fields:['Nom employé','Date des faits',tText('Description'),'Mesures attendues','Sanctions possibles']},
    {id:'tpl_conge',name:'Demande de congé',cat:'RH',icon:'🏖️',desc:'Formulaire de demande de vacances / congé',fields:['Nom employé','Type congé','Date début','Date fin','Nb jours',tText('Motif')]},
    {id:'tpl_expense',name:'Note de frais',cat:'Finance',icon:'🧾',desc:'Formulaire de remboursement de frais professionnels',fields:['Nom employé',tText('Date'),tText('Description'),tText('Montant'),'Justificatif',tText('Catégorie')]},
    {id:'tpl_eval',name:'Évaluation annuelle',cat:'RH',icon:'📊',desc:'Formulaire d\'évaluation des performances',fields:['Nom employé',tText('Période'),'Objectifs','Résultats','Points forts','Axes d\'amélioration','Note globale']},
  ];
...
<div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:10,marginBottom:18}}>{[{l:"Anciennete",v:anc.toFixed(1)+" ans",c:"#60a5fa"},{l:"Preavis employeur",v:semEmpl+" sem.",c:"#f87171"},{l:"Preavis travailleur",v:semTrav+" sem.",c:"#4ade80"},{l:"Indemnite empl.",v:fmt(indemnite),c:"#f87171"},{l:tText('Cout total'),v:fmt(coutTotal),c:"#fb923c"}].map((k,i)=><div key={i} style={{padding:"12px 14px",background:"rgba(198,163,78,.04)",borderRadius:10,border:"1px solid rgba(198,163,78,.08)"}}><div style={{fontSize:9,color:"#5e5c56",textTransform:"uppercase",letterSpacing:".5px"}}>{k.l}</div><div style={{fontSize:17,fontWeight:700,color:k.c,marginTop:4}}>{k.v}</div></div>)}</div>
...
export function SelfServiceMod({s,d}){
  ...
  const tabs=[{v:"fiches",l:tText('Fiches de paie'),ic:'📄'},{v:"conges",l:"Congés",ic:'📅'},{v:"docs",l:tText('Documents'),ic:'📋'},{v:"perso",l:"Données perso",ic:'👤'},{v:"contact",l:"Contact RH",ic:'💬'}];
  return <div>
      ...
      <div>
        ...
        {tab==='conges'&&<C>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:10,marginBottom:14}}>
            {[{l:"Légaux",v:20,u:20,c:'#4ade80'},{l:"Extra-légaux",v:2,u:5,c:'#60a5fa'},{l:"Récupération",v:0,u:3,c:'#a78bfa'},{l:tText('Ancienneté'),v:1,u:1,c:'#c6a34e'}].map((k,i)=>
              <div key={i} style={{padding:'12px',background:"rgba(198,163,78,.04)",borderRadius:8,textAlign:'center'}}>
                <div style={{fontSize:10,color:'#5e5c56'}}>{k.l}</div>
        ...
        {tab==='perso'&&<C>
          ...
          {[{l:tText('Nom'),v:`${emp.first} ${emp.last}`},{l:"NISS",v:emp.niss||'—'},{l:"Date naissance",v:emp.birth||'—'},{l:tText('Adresse'),v:`${emp.addr||''} ${emp.zip||''} ${emp.city||''}`},{l:"IBAN",v:emp.iban||'—'},{l:tText('Email'),v:emp.email||'—'},{l:tText('Téléphone'),v:emp.phone||'—'},{l:"Situation familiale",v:emp.civil||'—'},{l:"Personnes à charge",v:emp.children||0}].map((f2,i)=>
...
export function GEDMod({s,d}){
  ...
  return <div>
    ...
    <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:12,marginBottom:18}}>
      {[{l:"Documents total",v:totDocs,c:'#60a5fa'},{l:"Catégories",v:cats.length,c:'#a78bfa'},{l:tText('Travailleurs'),v:ae.length,c:'#c6a34e'},{l:"Stockage",v:Math.round(totDocs*0.2)+'MB',c:'#4ade80'}].map((k,i)=>
        <div key={i} style={{padding:'14px 16px',background:"rgba(198,163,78,.04)",borderRadius:10,border:'1px solid rgba(198,163,78,.08)'}}>
          <div style={{fontSize:10,color:'#5e5c56',textTransform:'uppercase',letterSpacing:'.5px'}}>{k.l}</div>
