...
export function PeppolMod({s,d}){
  const { t, lang } = useLang();const loisRef=LOIS_BELGES;const tvaRates=loisRef.tva||{standard:21,reduit:6,intermediaire:12};
...
export function RecoSalaireMod({s,d}){
  const { t, lang } = useLang();const ae= s?.emps||[];const n=ae.length;const [tab,setTab]=useState("analyse");const [poste,setPoste]=useState("admin");const mb=ae.reduce((a,e)=>a+(+e.gross||0),0);const moyBrut=n>0?mb/n:0;
...
export function APIDocMod({s,d}){
  const { t, lang } = useLang();const loisRef=LOIS_BELGES;
...
export function AutomationsMod({s,d}){
  const { t, lang } = useLang();const loisRef=LOIS_BELGES;const rmmmg=RMMMG;
...
export function TemplatesMod({s,d}){
  const { t, lang } = useLang();const loisRef=LOIS_BELGES;const rmmmgRef=RMMMG;const txOnss=TX_ONSS_E;
  const ae=(s?.emps||[]).filter(e=>e.status==='active'||!e.status);
...
export function PreavisMod({s,d}){
  const { t, lang } = useLang();const ae= s?.emps||[];const [tab,setTab]=useState("calcul");const [selEmp,setSelEmp]=useState(ae[0]?.id||"");const [ancManuelle,setAncManuelle]=useState(5);const [initPar,setInitPar]=useState("employeur");
...
export function AuditMod({s,d}){
  const { t, lang } = useLang();const ae= s?.emps||[];const n=ae.length;const [tab,setTab]=useState("global");const mb=ae.reduce((a,e)=>a+(+e.gross||0),0);
...
export function SelfServiceMod({s,d}){
  const { t, lang } = useLang();const ae=(s?.emps||[]).filter(e=>e.status==='active'||!e.status);const empNetData=ae.map(e=>({...e,netCalc:quickNet(+(e.gross||0)),pp:quickPP(+(e.gross||0)),onss:Math.round((+(e.gross||0))*TX_ONSS_W*100)/100}));const [sel,setSel]=useState(ae[0]?.id||'');const [tab,setTab]=useState('fiches');
