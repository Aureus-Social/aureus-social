...
const AGENT_QUICK={
  fr:[
    {i:'🏢',l:tText('Créer un dossier'),p:'Crée un dossier pour '},
    {i:'👤',l:tText('Ajouter travailleur'),p:'Ajoute un travailleur '},
    {i:'💰',l:tText('Simulation salaire'),p:'Calcule le salaire net pour un brut de '},
    {i:'📊',l:tText('Coût employeur'),p:"Combien me coûte un employé à "},
    {i:'⏱️',l:tText('Calcul préavis'),p:'Calcule le préavis pour une ancienneté de '},
    {i:'📋',l:tText('Info CP'),p:'Quelles sont les règles de la CP '},
    {i:'📄',l:tText('Procédure C4'),p:'Comment établir un C4 correctement?'},
    {i:'💶',l:tText('Calculer la paie'),p:'Calcule la paie de février pour '},
  ],
  nl:[
    {i:'🏢',l:tText('Dossier aanmaken'),p:'Maak een dossier aan voor '},
    {i:'👤',l:tText('Werknemer toevoegen'),p:'Voeg een werknemer toe '},
    {i:'💰',l:tText('Loonsimulatie'),p:'Bereken het nettoloon voor een bruto van '},
    {i:'📊',l:tText('Werkgeverskost'),p:'Hoeveel kost een werknemer aan '},
    {i:'⏱️',l:tText('Opzeg berekenen'),p:'Bereken de opzegtermijn voor een anciënniteit van '},
    {i:'📋',l:tText('Info PC'),p:'Wat zijn de regels van PC '},
  ],
  en:[
    {i:'🏢',l:tText('Create company'),p:'Create a file for '},
    {i:'👤',l:tText('Add worker'),p:'Add a worker '},
    {i:'💰',l:tText('Salary sim'),p:'Calculate net salary for gross of '},
    {i:'📊',l:tText('Employer cost'),p:'How much does an employee cost at '},
    {i:'⏱️',l:tText('Notice period'),p:'Calculate notice period for '},
    {i:'📋',l:tText('JC info'),p:'What are the rules of Joint Committee '},
  ],
};
...
function FloatingLegalAgent({onAction}){
  ...
  const labels={fr:{title:tText('Agent Juridique IA'),placeholder:tText('Votre question en droit social...'),disclaimer:'Info juridique indicative. Cas complexes → juriste.',clear:'Effacer'},
    nl:{title:tText('Juridische AI-Agent'),placeholder:tText('Uw vraag over sociaal recht...'),disclaimer:'Indicatieve juridische info. Complexe gevallen → jurist.',clear:'Wissen'},
    en:{title:tText('Legal AI Agent'),placeholder:tText('Your social law question...'),disclaimer:'Indicative legal info. Complex cases → jurist.',clear:'Clear'}};
        ...
        {msgs.map((m,i)=><div key={i} style={{display:'flex',justifyContent:m.role==='user'?'flex-end':'flex-start'}}>
          <div style={{
            ...
            {m.role==='assistant'&&<div style={{display:'flex',alignItems:'center',gap:6,marginBottom:8,paddingBottom:8,
              ...
              {m.executed&&<span style={{fontSize:9,padding:'1px 6px',borderRadius:8,background:'rgba(74,222,128,.15)',color:'#4ade80',fontWeight:600}}>{tText('✅ Action effectuée')}</span>}
            </div>}
            {m.content}
          </div>
        </div>)}
