...
export function PlanningCongesV3({s, defaultTab}){
  ...
  const emps=(s?.clients||[]).flatMap(c=>(c.emps||[]).map(e=>({...e,_cl:c.company?.name||tText('Client')})));
  ...
  const moisNoms=[tText('Janvier'),tText('Février'),tText('Mars'),tText('Avril'),tText('Mai'),tText('Juin'),tText('Juillet'),tText('Août'),tText('Septembre'),tText('Octobre'),tText('Novembre'),tText('Décembre')];
  ...
  return <div style={{padding:24}}>
    <h2 style={{fontSize:22,fontWeight:700,color:'#c6a34e',margin:'0 0 4px'}}>{tText('📅 Planning Congés')}</h2>
    <p style={{fontSize:12,color:'#888',margin:'0 0 20px'}}>{tText('Chevauchements + règles minimum présence + calendrier équipe')}</p>

    <div style={{display:'grid',gridTemplateColumns:'repeat(5,1fr)',gap:10,marginBottom:18}}>
      {[{l:tText('Effectif'),v:n,c:'#3b82f6'},{l:tText('Min présence'),v:minPresence+'%',c:'#c6a34e'},{l:tText('Jours alerte'),v:alertDays,c:alertDays>0?'#f87171':'#22c55e'},{l:tText('Mois'),v:moisNoms[month],c:'#c6a34e'},{l:tText('Jours ouvrables'),v:days.filter(d=>{const dow=new Date(year,month,d).getDay();return dow>0&&dow<6;}).length,c:'#888'}].map((k,i)=><div key={i} style={{padding:'12px 14px',background:'rgba(198,163,78,.04)',borderRadius:10,border:'1px solid rgba(198,163,78,.08)'}}><div style={{fontSize:9,color:'#5e5c56',textTransform:'uppercase'}}>{k.l}</div><div style={{fontSize:17,fontWeight:700,color:k.c,marginTop:4}}>{k.v}</div></div>)}
    </div>
    ...
    {tab==='calendrier'&&<div style={{overflowX:'auto'}}>
      <div style={{minWidth:Math.max(800,days.length*28+200)}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'180px repeat('+daysInMonth+',1fr)',gap:1,marginBottom:2}}>
          <div style={{padding:4,fontSize:9,color:'#888'}}>{tText('Employé')}</div>
        ...
        <div style={{display:'grid',gridTemplateColumns:'180px repeat('+daysInMonth+',1fr)',gap:1,marginBottom:4,padding:'4px 0',borderBottom:'2px solid rgba(198,163,78,.15)'}}>
          <div style={{padding:4,fontSize:9,fontWeight:700,color:'#c6a34e'}}>{tText('% Présence')}</div>
...
export function BilanSocialV3({s}){
  ...
  const cdi=emps.filter(e=>(e.contractType||'').toUpperCase()!==tText('CDD')).length;
  const cdd=n-cdi;
  ...
  const sections=[
    ...
    {num:'II',titre:'Tableau mouvement du personnel',rubrique:'Rubrique 200',items:[
      ...
      {code:'2003',label:tText('Taux de rotation'),value:n>0?((Math.round(n*0.10)+Math.round(n*0.15))/(2*n)*100).toFixed(1)+'%':'0%',unit:''},
    ]},
    {num:'III',titre:'Utilisation de mesures en faveur de l\'emploi',rubrique:'Rubrique 300',items:[
      {code:'3001',label:tText('Premiers engagements (réduction ONSS)'),value:n<=6?n:0,unit:''},
      ...
      {code:'3005',label:tText('Stages'),value:Math.round(n*0.04),unit:''},
    ]},
    {num:'IV',titre:tText('Formation'),rubrique:'Rubrique 580',items:[
      {code:'5801',label:tText('Nombre de travailleurs ayant suivi une formation'),value:Math.round(n*0.7),unit:''},
      {code:'5801a',label:'→ Hommes',value:Math.round(hommes*0.7),unit:''},
      {code:'5801b',label:'→ Femmes',value:Math.round(femmes*0.7),unit:''},
      {code:'5802',label:tText('Heures de formation'),value:fi(Math.round(n*0.7)*24),unit:'h'},
      {code:'5803',label:tText('Coût net formation'),value:fmt(Math.round(n*0.7)*24*50)+' €',unit:''},
      {code:'5804',label:tText('Ratio formation / masse salariale'),value:masseBrut>0?((Math.round(n*0.7)*24*50)/(masseBrut*12)*100).toFixed(2)+'%':'0%',unit:''},
    ]},
  ];
  ...
  return <div style={{padding:24}}>
    ...
    {tab==='pyramide'&&<C title="Pyramide des âges">
      ...
      <div style={{display:'flex',justifyContent:'center',gap:20,marginTop:10}}>{[{l:tText('Hommes'),c:'#3b82f6'},{l:tText('Femmes'),c:'#ec4899'}].map((g,i)=><div key={i} style={{display:'flex',alignItems:'center',gap:6}}><div style={{width:12,height:12,borderRadius:2,background:g.c}}/><span style={{fontSize:10,color:'#888'}}>{g.l}</span></div>)}</div>
    </C>}
...
export function AnalyticsV3({s}){
  ...
  return <div style={{padding:24}}>
    ...
    <div style={{display:'grid',gridTemplateColumns:'repeat(6,1fr)',gap:10,marginBottom:18}}>
      {[{l:tText('Effectif total'),v:n,c:'#3b82f6'},{l:tText('Masse salariale/mois'),v:fmt(masseBrut)+' €',c:'#c6a34e'},{l:tText('Coût total/mois'),v:fmt(masseBrut*(1+TX_ONSS_E))+' €',c:'#f87171'},{l:tText('Salaire médian'),v:fmt(median)+' €',c:'#22c55e'},{l:tText('CDI / CDD'),v:cdi+' / '+(n-cdi),c:'#a855f7'},{l:tText('Temps partiel'),v:tp+' ('+Math.round(tp/Math.max(n,1)*100)+'%)',c:'#06b6d4'}].map((k,i)=><div key={i} style={{padding:'10px 12px',background:'rgba(198,163,78,.04)',borderRadius:10,border:'1px solid rgba(198,163,78,.08)'}}><div style={{fontSize:8,color:'#5e5c56',textTransform:'uppercase'}}>{k.l}</div><div style={{fontSize:14,fontWeight:700,color:k.c,marginTop:3}}>{k.v}</div></div>)}
    </div>
    ...
    {tab==='clients'&&<C title="Analyse par client">
      <div style={{display:'grid',gridTemplateColumns:'200px repeat(4,1fr)',gap:4,padding:'6px 0',borderBottom:'2px solid rgba(198,163,78,.15)',fontSize:9,fontWeight:700,color:'#c6a34e'}}>
        <div>{tText('Client')}</div><div>{tText('Effectif')}</div><div>{tText('Masse salariale')}</div><div>{tText('Coût total')}</div><div>{tText('Salaire moyen')}</div>
      </div>
    ...
    {tab==='kpi'&&<C title="KPIs RH">
      {[
        {l:tText('Taux de rotation estimé'),v:n>0?(Math.round(n*0.10)/(n)*100).toFixed(1)+'%':'0%',c:'#fb923c',b:'< 10% = bon, 10-15% = attention, > 15% = critique'},
        ...
        {l:tText('Ratio ONSS / masse salariale'),v:(TX_ONSS_E*100).toFixed(2)+'%',c:'#f87171',b:'Taux légal 2026'},
        {l:tText('Taux temps partiel'),v:n>0?Math.round(tp/n*100)+'%':'0%',c:'#06b6d4',b:'Tendance Belgique: ~25%'},
        {l:tText('Taux CDI'),v:n>0?Math.round(cdi/n*100)+'%':'0%',c:'#22c55e',b:'Norme: > 85% = stabilité'},
        {l:tText('Indice de diversité genre'),v:n>0?Math.min(emps.filter(e=>(e.gender||'').toLowerCase()==='m').length,emps.filter(e=>(e.gender||'').toLowerCase()==='f').length)/Math.max(n/2,1)*100:'N/A',c:'#ec4899',b:'100% = parité parfaite'},
        {l:tText('Masse salariale annuelle'),v:fmt(masseBrut*12)+' €',c:'#c6a34e',b:'Projection sur 12 mois hors provisions'},
...
export function ImportCSVV3({s}){
  ...
  const targetFields=[
    ...
    {id:'niss',label:tText('NISS'),required:true,validate:v=>/^\d{2}\.\d{2}\.\d{2}[-]\d{3}[-\.]\d{2}$/.test(v)||/^\d{11}$/.test(v)},
    {id:'birthDate',label:tText('Date de naissance'),required:true,validate:v=>{const d=new Date(v);return!isNaN(d.getTime());}},
    {id:'startDate',label:tText('Date d\'entrée'),required:true,validate:v=>{const d=new Date(v);return!isNaN(d.getTime());}},
    {id:'gross',label:tText('Salaire brut'),required:true,validate:v=>!isNaN(+v)&&+v>0},
    {id:'contractType',label:tText('Type contrat'),required:false,validate:v=>!v||[tText('CDI'),tText('CDD'),'INTERIM','ETUDIANT','FLEXI'].includes((v||'').toUpperCase())},
    {id:'regime',label:tText('Régime (%)'),required:false,validate:v=>!v||(!isNaN(+v)&&+v>0&&+v<=100)},
    {id:'function',label:tText('Fonction'),required:false,validate:()=>true},
    {id:'email',label:tText('Email'),required:false,validate:v=>!v||/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(v)},
    {id:'gender',label:tText('Genre (M/F)'),required:false,validate:v=>!v||['M','F','X'].includes((v||'').toUpperCase())},
  ...
  const parseCSV=(text)=>{
    ...
    hdrs.forEach((h,i)=>{
      ...
      else if(hl.includes(tText('contrat'))||hl.includes('contract'))autoMap['contractType']=i;
      ...
      else if(hl.includes(tText('email'))||hl.includes('mail'))autoMap[tText('email')]=i;
  ...
  return <div style={{padding:24}}>
    ...
    <p style={{fontSize:12,color:'#888',margin:'0 0 20px'}}>{tText('Mapping colonnes intelligent + validation NISS/dates + preview erreurs')}</p>

    <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:10,marginBottom:18}}>
      {[{l:tText('Colonnes détectées'),v:headers.length,c:'#3b82f6'},{l:tText('Lignes'),v:rows.length,c:'#c6a34e'},{l:tText('Champs mappés'),v:Object.keys(mapping).length+'/'+targetFields.length,c:'#22c55e'},{l:tText('Erreurs'),v:errors.length,c:errors.length>0?'#f87171':'#22c55e'}].map((k,i)=><div key={i} style={{padding:'12px 14px',background:'rgba(198,163,78,.04)',borderRadius:10,border:'1px solid rgba(198,163,78,.08)'}}><div style={{fontSize:9,color:'#5e5c56',textTransform:'uppercase'}}>{k.l}</div><div style={{fontSize:17,fontWeight:700,color:k.c,marginTop:4}}>{k.v}</div></div>)}
    </div>
    ...
    {tab==='mapping'&&<C title="🔗 Mapping des colonnes">
      {headers.length===0?<div style={{color:'#888',padding:20,textAlign:'center'}}>{tText('Importez d\'abord un fichier CSV')}</div>:
      ...
      {headers.length>0&&<button onClick={validateData} style={{marginTop:12,padding:'10px 24px',borderRadius:8,border:'none',background:'rgba(34,197,94,.15)',color:'#22c55e',fontSize:13,fontWeight:600,cursor:'pointer'}}>{tText('✓ Valider les données')}</button>}
    </C>}

    {tab==='preview'&&<C title="👁 Aperçu des données mappées">
      {rows.length===0?<div style={{color:'#888',padding:20,textAlign:'center'}}>{tText('Importez d\'abord un fichier CSV')}</div>:
    ...
    {tab==='erreurs'&&<C title={"⚠ Erreurs de validation ("+errors.length+")"} color={errors.length>0?"#f87171":"#22c55e"}>
      {!validated?<div style={{color:'#888',padding:20,textAlign:'center'}}>{tText('Lancez la validation depuis l\'onglet Mapping')}</div>:
    ...
    {tab==='format'&&<C title="📋 Format CSV attendu">
      ...
      <div style={{fontSize:12,fontWeight:600,color:'#c6a34e',marginBottom:8}}>{tText('Champs obligatoires *')}</div>
...
export function ContratsLegauxV3({s}){
  ...
  return <div style={{padding:24}}>
    <h2 style={{fontSize:22,fontWeight:700,color:'#c6a34e',margin:'0 0 4px'}}>{tText('📝 Contrats Légaux')}</h2>
    <p style={{fontSize:12,color:'#888',margin:'0 0 20px'}}>{tText('CDI, CDD, Étudiant, Intérim, Flexi-job, Remplacement — Clauses spécifiques')}</p>
    ...
    {tab==='contrats'&&<div>
      {CONTRAT_TYPES.map(ct=>{const isExp=expanded[ct.id];return <div key={ct.id} style={{marginBottom:8}}>
        ...
        {isExp&&<div style={{padding:16,background:'rgba(198,163,78,.02)',border:'1px solid rgba(198,163,78,.08)',borderTop:'none',borderRadius:'0 0 10px 10px'}}>
          ...
          <div style={{marginTop:10}}><div style={{fontSize:9,fontWeight:700,color:'#3b82f6',textTransform:'uppercase',marginBottom:4}}>{tText('Spécificités')}</div>
    ...
    {tab==='detail'&&<C title={sel.nom}>
      ...
      <div style={{marginTop:12}}><div style={{fontSize:11,fontWeight:700,color:'#3b82f6',marginBottom:6}}>{tText('Spécificités')}</div>{sel.specificites.map((c,j)=><div key={j} style={{fontSize:11,color:'#ccc',padding:'3px 0'}}>• {c}</div>)}</div>
    </C>}
    ...
    {tab==='comparatif'&&<C title="Comparatif des types de contrats">
      <div style={{overflowX:'auto'}}>
        <div style={{display:'grid',gridTemplateColumns:'140px repeat('+CONTRAT_TYPES.length+',1fr)',gap:4,minWidth:800}}>
          ...
          {[
            {label:tText('ONSS employeur'),key:'onss_e',vals:['25.07%','25.07%','2.71%','Via agence','28%','25.07%']},
