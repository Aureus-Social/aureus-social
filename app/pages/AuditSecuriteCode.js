...
function AuditSecuriteTab({s}) {
  ...
  const checks = [
    { cat:'Authentification', items:[
      ...
      { id:'auth2', label:'Session JWT vérifiée côté serveur', status:'ok', detail:'getAuthUser() vérifie Bearer token sur 39 routes API — supabase.auth.getUser()' },
      { id:'auth3', label:'2FA / MFA (TOTP)', status:'warning', detail:'Module mfa.js créé — intégration dans login.js en attente' },
      { id:'auth4', label:'Timeout de session (30min)', status:'ok', detail:'useEffect actif dans layout-client.js — resetTimer sur 6 événements + audit log' },
    ]},
    { cat:'Données sensibles', items:[
      { id:'data1', label:'NISS/IBAN chiffrés en base', status:'ok', detail:'AES-256-GCM via encryptField() dans /api/employees POST/PUT — RGPD Art.32 conforme' },
      { id:'data2', label:'IBAN masqué dans les vues', status:'ok', detail:'mask.iban() appliqué dans EmployeeHub — BE76 **** **** 3456' },
      { id:'data3', label:'RLS Supabase (multi-tenant)', status:'warning', detail:'Script SQL prêt (rls-setup.sql) — à exécuter dans Supabase Dashboard SQL Editor' },
      { id:'data4', label:tText('Logs d\'accès aux données'), status:'ok', detail:'Table audit_log + /api/audit route — 47 insertions dans les APIs (CREATE/UPDATE/DELETE)' },
    ]},
    ...
    { cat:'Code', items:[
      ...
      { id:'code2', label:'Dépendances auditées (npm audit)', status:'ok', detail:'GitHub Actions security-audit.yml — scan lundi 05h00 UTC + alerte email Resend si critique' },
