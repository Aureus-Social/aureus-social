    ...
    {(s.pays||[]).length>0&&<C  style={{marginTop:20,padding:0,overflow:'hidden'}}>
      <div style={{padding:'14px 18px',borderBottom:'1px solid rgba(139,115,60,.1)',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
        ...
        <div style={{display:'flex',gap:8}}>
          {(s.pays||[]).some(p=>(!p.gross||p.gross===0)&&(!p.ename||p.ename==='undefined undefined'))&&<button onClick={async()=>{if(confirm('Supprimer toutes les fiches en erreur (undefined / 0€) ?')){const badIds=(s.pays||[]).filter(p=>(!p.gross||p.gross===0)&&(!p.ename||p.ename==='undefined undefined')).map(p=>p.id);d({type:'DEL_PAYS_BATCH',ids:badIds});if(supabase&&badIds.length){const{data:{user}}=await supabase.auth.getUser().catch(()=>({data:{}}));if(user?.id)supabase.from('fiches_paie').delete().in('id',badIds).eq('user_id',user.id).catch(()=>{});}}}} style={{padding:'6px 12px',background:'#7f1d1d',color:'#fca5a5',border:'1px solid #991b1b',borderRadius:6,fontSize:11,fontWeight:600,cursor:'pointer'}}>🗑 Supprimer les fiches en erreur ({(s.pays||[]).filter(p=>(!p.gross||p.gross===0)&&(!p.ename||p.ename==='undefined undefined')).length})</button>}
          {(s.pays||[]).length>0&&<button onClick={async()=>{if(confirm('⚠ Supprimer TOUTES les fiches de paie ? Cette action est irréversible.')){d({type:'SET_PAYS',data:[]});if(supabase){const{data:{user}}=await supabase.auth.getUser().catch(()=>({data:{}}));if(user?.id)supabase.from('fiches_paie').delete().eq('user_id',user.id).catch(()=>{});}}}} style={{padding:'6px 12px',background:'#1e293b',color:'#94a3b8',border:'1px solid #334155',borderRadius:6,fontSize:11,fontWeight:500,cursor:'pointer'}}>{'Tout effacer'}</button>}
        </div>
      </div>
      <Tbl cols={[
        ...
        {k:'x',l:"",a:'center',r:r=><button onClick={async(e)=>{e.stopPropagation();if(confirm('Supprimer cette fiche de '+r.ename+' ('+r.period+') ?')){d({type:'DEL_P',id:r.id});if(supabase&&r.id){const{data:{user}}=await supabase.auth.getUser().catch(()=>({data:{}}));if(user?.id)supabase.from('fiches_paie').delete().eq('id',r.id).eq('user_id',user.id).catch(()=>{});}}}} style={{padding:'4px 8px',background:'transparent',color:'#ef4444',border:'1px solid rgba(239,68,68,.3)',borderRadius:4,fontSize:10,cursor:'pointer',fontWeight:600,opacity:.7}} onMouseEnter={e=>e.target.style.opacity=1} onMouseLeave={e=>e.target.style.opacity=.7}>🗑</button>},
      ]} data={s.pays}/>
    </C>}
  </div>;
}
